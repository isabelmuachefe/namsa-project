    ...
    const load = async () => {
      try {
        ...
        const computed: DashboardStats = {
          ...
          approvedProfiles: allProfiles.filter((p: any) => p.status?.statusName === 'APPROVED').length,
          rejectedProfiles: allProfiles.filter((p: any) => p.status?.statusName === 'REJECTED').length,
          recentActivity: [],
        } as any;
